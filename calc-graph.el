...
(defun calc-graph-compute-2d ()
  (if (setq calc-graph-yvec (eq (car-safe calc-graph-yvalue) 'vec))
	...
	(if (setq calc-graph-xvec (eq (car-safe calc-graph-xvalue) 'vec))
		...
		(error "%s and %s have different lengths" 
		       calc-graph-xname calc-graph-yname))
...
(defun calc-graph-recompute-2d ()
  ...
  (while (>= (setq calc-graph-numsteps (1- calc-graph-numsteps)) 0)
	...
	(progn
	  ...
	  (setcdr calc-graph-ycacheptr 
		  (cons (cons calc-graph-xval 
			      (math-evaluate-expr calc-graph-yvalue))
			(cdr calc-graph-ycacheptr)))))
    (setq calc-graph-ycacheptr (cdr calc-graph-ycacheptr))
    (if calc-graph-xvec
	(setq calc-graph-yvector 
	      (cons (cdr (car calc-graph-ycacheptr)) calc-graph-yvector))
...
(defun calc-graph-compute-3d ()
  (if (setq calc-graph-yvec (eq (car-safe calc-graph-yvalue) 'vec))
	  ...
	  (progn
		...
		(or (= (1- (length calc-graph-xvalue)) calc-graph-numsteps)
		  ...
		  (setq calc-graph-xvalue (calcFunc-index calc-graph-numsteps
						...
						(1- calc-graph-numsteps))))
		(if (math-realp calc-graph-xvalue)
		    (setq calc-graph-xvalue 
			  (calcFunc-index calc-graph-numsteps calc-graph-xvalue 1))
		  (error "%s is not a suitable basis for %s" 
			 calc-graph-xname calc-graph-yname))))
		...
		(if (math-realp calc-graph-y3value)
		    (setq calc-graph-y3value 
			  (calcFunc-index calc-graph-numsteps3 calc-graph-y3value 1))
		  (error "%s is not a suitable basis for %s" 
	    ...
	    (while (setq calc-graph-xvalue 
	      ...
	      (setq calc-graph-xp 
		    (nconc calc-graph-xp 
			   (make-list 
			    (1+ calc-graph-numsteps3) (car calc-graph-xvalue)))
		    calc-graph-yp 
		    (nconc calc-graph-yp 
	...
	(let ((math-arglist nil))
	  ...
	  (or (cdr math-arglist)
	      (error "%s does not contain enough unassigned variables" 
		     calc-graph-yname))
	  (and (cdr (cdr math-arglist))
	       (error "%s contains too many variables: %s" 
		      calc-graph-yname math-arglist))
    ...
    (while (setq calc-graph-xvalue (cdr calc-graph-xvalue))
      (setq calc-graph-xp 
	    (nconc calc-graph-xp 
		   (make-list (1+ calc-graph-numsteps3) (car calc-graph-xvalue)))
	    calc-graph-yp (nconc calc-graph-yp 
				 (cons 0 (copy-sequence (cdr calc-graph-y3value))))
	...
	(setq math-working-step-2 (1+ math-working-step-2)
	      ...
	      calc-graph-zp 
	      (cons (math-evaluate-expr calc-graph-yvalue) calc-graph-zp))))
