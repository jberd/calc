...
(defvar calc-empty-stack '((top-of-stack 1 nil))
  "The contents of an empty stack.")
...
;;; SXEmacs might not handle these symbols the same way.
...
(defun calc-get-window ()
  (or (derived-mode-p 'calc-mode)
      (if calc-main-buffer
	  (get-buffer-window calc-main-buffer)
	(let ((buf (get-buffer "*Calculator*")))
	  (if buf
	...
	(with-current-buffer buf
	  (let ((aligned (calc-check-trail-aligned))
	    ...
	    (let ((win (get-buffer-window buf)))
	      ...
	      (calc-trail-here)))))) ; Shouldn't be necessary.
  val)
...
;;; Right now, use the calcDigit-start from XEmacs
  ...
  (calc-wrapper
   (if (or calc-algebraic-mode
     ...
     (let* ((calc-digit-value nil)
	    ...
	    (buf (if t ;calc-emacs-type-lucid
		     (catch 'calc-foo
...
;;; SXEmacs doesn't have minibuffer-prompt-end or minibuffer-contents,
;;; for now use point-min and buffer-string
...
(defun calcDigit-nondigit ()
  ...
  (if (and (null calc-digit-value) (> (buffer-size) 0))
    ...
    (or (memq last-command-char '(?  ?\r))
	(progn (setq prefix-arg current-prefix-arg)
	       (calc-unread-command (if (and (eq last-command-char ?\e)
					     (>= (char-int last-input-char)
						 128))
					last-input-char
				      nil))))
    (exit-minibuffer)))
...
;;    L  ...or normalized vector element list (without "vec")
...
;;; Compute the integer (quotient . remainder) of A and B, which are
;;; integers.  Type and consistency of truncation is undefined
...
(defun calc-unread-command (&optional input)
  (if (featurep 'xemacs)
      (setq unread-command-event
	    (if (characterp input) (character-to-event input)
	      (or input last-command-event)))
    (push (or input last-command-char) unread-command-events)))
