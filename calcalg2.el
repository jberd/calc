...
(defvar math-trace-buffer)

(defmacro math-tracing-integral (&rest parts)
  `(and math-trace-buffer
	(with-current-buffer math-trace-buffer
	  (goto-char (point-max))
	  (and (bolp)
  ...
  (cond
   ...
   (t
    ...
    (let* ((math-max-integral-limit (or (and (natnump var-IntegLimit)
	   ...
	   (math-trace-buffer (get-buffer "*Trace*"))
	   ...
	   (math-good-parts nil)
	   (res
	    (if math-trace-buffer
		(let ((calcbuf (current-buffer))
		      (calcwin (selected-window)))
		  (unwind-protect
		      (progn
			(if (get-buffer-window math-trace-buffer)
			    (select-window (get-buffer-window math-trace-buffer)))
			(set-buffer math-trace-buffer)
			(goto-char (point-max))
