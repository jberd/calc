...
(defun calc-describe-key (key &optional briefly)
  ...
  (let ((defn (if (eq (key-binding key) 'calc-dispatch)
    ...
    (while (or (equal (key-description key) "I") 
	       (equal (key-description key) "H"))
      (if (equal (key-description key) "I")
	  (setq inv (not inv))
	(setq hyp (not hyp)))
            ...
            (while (re-search-forward "\n\\* \\(.*\\): " nil t)
              ...
              (if (and (not (string-match "<[1-9]+>" entry))
                  ...
                  (unless (assoc entry entrylist)
                    (setq entrylist (cons (list entry) entrylist))))))
        (error nil))
