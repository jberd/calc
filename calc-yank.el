...
(defun calc-copy-to-register (register &optional delete-flag)
...
(defun calc-append-to-register (register &optional delete-flag)
...
(defun calc-prepend-to-register (register &optional delete-flag)
...
(defun calc-do-grab-rectangle (top bot arg &optional reduce)
  ...
  (let* ((col1 (save-excursion (goto-char top) (current-column)))
	...
	(save-excursion
	  (unless (= col1 0)
	    (error 
	     "Point and mark must be at beginning of line, or define a rectangle"))
	  (goto-char top)
	  (while (< (point) bot)
	    (setq pt (point))
	    (forward-line 1)
...
(defun calc-copy-to-buffer (nn)
  ...
  (if (zerop (calc-stack-size))
      (message "The Calc stack is empty.")
