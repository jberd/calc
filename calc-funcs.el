    ...
    (math-with-extra-prec 3
      ...
      (cond ((math-zerop x)
	    ...
	    (t
	     ...
	     (let* ((j (logior (+ v (math-isqrt (* 40 v))) 1))
		    (two-over-x (math-div 2 x))
		    (jsum nil)
		    (bjp '(float 0 0))
		    (sum '(float 0 0))
...
(defun math-besJ1 (x &optional yflag)
  (cond ((and (math-negp (calcFunc-re x)) (not yflag))
	...
	((Math-lessp '(float 8 0) (math-abs-approx x))
	 (let* ((z (math-div '(float 8 0) x))
		(y (math-sqr z))
		(xx (math-add x '(float -2356194491 -9)))
		(a1 (math-poly-eval y
                          (list
                           '(float -240337019 -15)
                           '(float 2457520174 -15)
	 ...
	 (let ((y (math-sqr x)))
	    ...
	    (math-div (math-poly-eval y
		      ...
		      (math-poly-eval y
			     ...
			     '(float 7189466438 -3)
			     '(float 7452499648 -1)
			     '(float 400765442690 -1))))
	    (math-mul '(float 636619772 -9)
	   ...
	   (math-add
	    (math-mul
	     ...
	     (math-div (math-poly-eval y
			      ...
			      '(float -4237922726 -3)
			      '(float 7349264551 -1)
			      '(float -515343813900 -1)
			      '(float 12752743900000 -1)
			      '(float -49006049430000 -1)))
		       (math-poly-eval y
                             (list
                              '(float 1 0)
			      '(float 3549632885 -7)
			      ...
			      '(float 2245904002 -2)
			      '(float 37336503670 -1)
			      '(float 4244419664000 -1)
			      '(float 249958057000000 -1)))))
	    (math-mul '(float 636619772 -9)
