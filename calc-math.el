...
;;;
(defvar calc-use-bigfr (fboundp 'bigfr-get-precision)
  "Non-nil if Calc has bigfr support.")

(defun math-calcnum-to-bigfr (x prec)
  "Convert X to a bigfr number, with precision PREC."
  (cond ((integerp x)
	 x)
	((eq (car x) 'frac)
	 (bigfr (// (nth 1 x) (nth 2 x)) prec))
	((eq (car x) 'float)
	 (if (positivep (nth 2 x))
	     (bigfr (* (nth 1 x) (10^ (nth 2 x))) prec)
	   (bigfr (// (nth 1 x) (10^ (- (nth 2 x)))) prec)))))

(defun math-bigfr-to-calcnum (x)
  "Convert X to a Calc number."
  (if (integerp x) ;; Possible?
      x
    (let ((str (number-to-string x))
	  ...
	  ;;; Check for 2.34e5
	  (setq expn (string-to-number (math-match-substring str 2))
		str  (math-match-substring str 1)))
      (if (string-match "^\\(-?[0-9]*\\)\\.\\([0-9]*\\)$" str)
	  ;;; Check str for 2.34
	  (setq frac (math-match-substring str 2)
		str (math-match-substring str 1)))
      (list 'float (string-to-number (concat str frac))
	    (+ expn (- (length frac)))))))

(defun math-use-emacs-fn (fn x)
  "Use the native Emacs function FN to evaluate the Calc number X 
to precision PREC, if Emacs has bigfr support.  If this can't be done, 
return NIL."
  (if calc-use-bigfr
      (let* ((prec (+ 4 (ceiling (log (10^ calc-internal-prec) 2))))
	     (default-real-precision prec)
	     (fx (math-calcnum-to-bigfr x prec)))
	(and
	 fx
	 (math-bigfr-to-calcnum (funcall fn fx))))))
...
(defun math-sqrt (a)
  (or
   ...
   (and (eq (car-safe a) 'frac)
	(let* ((num-isqrt (math-isqrt (nth 1 a)))
	       (den-isqrt (math-isqrt (nth 2 a)))
	       (num-sqrp (= (* num-isqrt num-isqrt) (nth 1 a)))
	       (den-sqrp (= (* den-isqrt den-isqrt) (nth 2 a))))
	  (if (and num-sqrp den-sqrp)
	      (list 'frac num-isqrt den-isqrt)
	    (if calc-symbolic-mode
		(if (or num-sqrp den-sqrp)
		    (math-div (if num-sqrp
				  num-isqrt (list 'calcFunc-sqrt (nth 1 a)))
			      (if den-sqrp
				  den-isqrt (list 'calcFunc-sqrt (nth 2 a))))
		  (list 'calcFunc-sqrt a))
	      (math-sqrt-float (math-float a)
			       (math-div (math-float num-isqrt) den-isqrt))))))
   (and (eq (car-safe a) 'float)
	(if calc-symbolic-mode
	    (if (= (% (nth 2 a) 2) 0)
		(let ((res (math-isqrt (nth 1 a))))
		  (if (=  (* res res) (nth 1 a))
		      (math-make-float res 
				       (/ (nth 2 a) 2))
     ...
     (t
      (if (null guess)
          (let ((ldiff (- (math-numdigs (nth 1 a)) 6)))
            ...
            (setq guess (math-make-float (math-isqrt
...
(defconst math-approx-ln-10
  '(float 2302585092994045684018 -21)
...
(defconst math-approx-ln-2
  '(float 693147180559945309417 -21)
...
(defun calcFunc-sinh (x)   ; [N N] [Public]
  (cond ((eq x 0) 0)
	...
	((and (Math-realp x) calc-use-bigfr)
	 (math-use-emacs-fn 'sinh x))	 
	((Math-numberp x)
