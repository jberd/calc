...
(defvar math-embed-the-language)
(defvar math-embed-the-display-just)
(defconst calc-embedded-mode-vars '(("twos-complement" . calc-twos-complement-mode)
				    ...
				    ("language" . math-embed-the-language)
				    ("plain" . calc-show-plain)
				    ("break" . calc-line-breaking)
				    ("justify" . math-embed-the-display-just)
    ...
    (if calcbuf
        (with-current-buffer calcbuf
          ...
          (let ((v calc-embedded-mode-vars))
            (while v
              (let ((var (cdr (car v))))
                (unless (memq var '(math-embed-the-language math-embed-the-display-just))
                  (setq modes
...
(defun calc-embedded-set-modes (gmodes modes local-modes &optional temp)
  (let ((math-embed-the-language (calc-embedded-language))
	(math-embed-the-display-just (calc-embedded-justify))
	(v gmodes)
	(changed nil)
	found value)
    (while v
    ...
    (and changed (not (eq temp t))
	 (progn
	   (calc-embedded-set-justify math-embed-the-display-just)
	   (calc-embedded-set-language math-embed-the-language)))
...
(defun calc-embedded-find-modes ()
  (let ((case-fold-search nil)
    ...
    (while (and no-defaults (search-backward "[calc-" nil t))
      ...
      (backward-char 6))
    (goto-char save-pt)
    (unless (assq 'math-embed-the-language modes)
      (let ((lang (assoc major-mode calc-language-alist)))
        (if lang
            (setq modes (cons (cons 'math-embed-the-language (cdr lang))
                              modes)))))
...
(defun calc-embedded-update (info which need-eval need-display
  ...
  (let* ((calc-embed-prev-modes nil)
	...
	(cond ((eq (car (car calc-embed-prev-modes)) 'math-embed-the-language)
	       (if need-display
		   (calc-embedded-set-language (cdr (car calc-embed-prev-modes)))))
	      ((eq (car (car calc-embed-prev-modes)) 'math-embed-the-display-just)
	       (if need-display
...
(defun calc-embedded-modes-change (vars)
  (if (eq (car vars) 'calc-language) (setq vars '(math-embed-the-language)))
  (if (eq (car vars) 'calc-display-just) (setq vars '(math-embed-the-display-just)))
  (while (and vars
  ...
  (if (and vars calc-mode-save-mode (not (eq calc-mode-save-mode 'save)))
	...
	(let* ((save-mode calc-mode-save-mode)
	       ...
	       (math-embed-the-language (calc-embedded-language))
	       (math-embed-the-display-just (calc-embedded-justify))
